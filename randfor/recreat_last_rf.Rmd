...
Note that the `echo = FALSE` parameter was added to the code chunk to prevent printing of the R code that generated the plot.
 i need first to creat my dataframe
...
path_to_scripts<-"D:/scripts_for_adding_netwrok/randomforest/scripts_for_rf"
...
#### the actuall run (if the user choose to run from the start)
  for (i in 1:num_of_pop){
    ...
    frq_bl_per_fly(dir[i,1],path_to_scripts)
  }
...
for(i in 1:num_of_pop){
  if(basename(dirname(dir[i,1])) == "Males"){
    setwd(dir[i,1])
    all_features<-read.csv("combined per fly.csv")
  ...
  if(basename(dirname(dir[i,1])) == "Females"){
    setwd(dir[i,1])
    all_features<-read.csv("combined per fly.csv")
    #getting the colom names - getting the feature name e.g aggregation 
    feature_names<-all_features %>%
