...
##this script calculate at evry movie the avg degree for evey 120 frame(can be chnagable in numbers) and visulaize it 
...
#for acast so it will sum it as 1 
all<-cbind(all,1)
...
for(i in 1:(length(numbers)-1)){
  ...
  mat_of_termins_and_onset<-as.matrix(all)
  temp_between_values<-all %>% filter(c(numbers[i],numbers[i+1]) %overlaps%  (all[,3:4]))
  ...
  value_sum_temp<-data.frame(begin=numbers[i],end=numbers[i+1],avg_value=mean(deg))
  value_sum_df<- bind_rows(value_sum_df,value_sum_temp)
  }else{
  value_sum_temp<-data.frame(begin=numbers[i],end=numbers[i+1],avg_value=0)
  value_sum_df<- bind_rows(value_sum_df,value_sum_temp)
  }
  }
...
data$value<-value_sum_df$avg_value
...
#with(value_sum_df, hist(avg_value))
...
#ggsave(filename = name_for_file,device="pdf",path="C:/Users/barakli8/OneDrive - Bar Ilan University/Lital/weekly presentation/26.05")
```
...
library(igraph)
library(R.matlab)
path ="D:/allGroups/Males_Singels"
setwd(path)
number_of_flys<-10
number_of_frame<-40
...
for(num_of_movie in 1:length(dir)){
  dir[num_of_movie]
  #it is under the assumtion that there is 10 flys and they are orgenized in spcific way from the Allinteraction.mat
  mat <- (readMat(paste0(path,dir[num_of_movie], '/','Allinteraction.mat'))) # read each MAT file
...
if(num_of_movie==1){
  ...
  total_data_for_Graph<-rbind(total_data_for_Graph,data)
}
...
# ggplot(avarge_per_frames,aes(x=Frame,y=Value))+geom_point()
 ...
 name_for_file<-paste0(basename(path),number_of_frame,"_frames_all_movies_avg_degree_with_avg_red.pdf")
...
```{r}
total_data_for_Graph$Frame <- factor(total_data_for_Graph$Frame, levels=unique(total_data_for_Graph$Frame))



test<-aggregate(total_data_for_Graph[,2], list(total_data_for_Graph$Frame), mean)
