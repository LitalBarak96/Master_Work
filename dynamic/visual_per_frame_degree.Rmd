...
#for each group calculating mean and ploting of all movies per frame 
#using the csv of per frame
```{r}
...
p<-ggplot()+geom_line(data = only_values_all_movies, mapping = aes(x = X, y = Value,group=group,color=group)) +geom_point(data=mean_every_row, aes(x=Frame, y=Value), color='black', size = 2,alpha = 0.15)

 p<-p+theme(text = element_text(size=12),
        axis.text.x = element_text(angle=90, hjust=1)) +ggtitle(paste0(basename(path_to_group)," density per frame all movies with avarge")) +
...
#if i need to do statisctic this is a start ish?
