...
Connected Components with linear regression

claculating in the top the cc and combing him to creat linear regression
didnt do well, i think it is becuase cc is not very good 
...
all_other_features<-read.csv("D:/allGroups/Females_Mated/Assa_Female_Mated_Unknown_RigA_20211025T120646/per_framefeatures_sum_allflies.csv")[1:26997,]
  ...
  cc<-cc_frame_and_value$cc
    ...
    data_cor <- cor(data[ , colnames(data) != "cc"],  # Calculate correlations
                data$cc)
    
  temp_degree_corr <- as.data.frame(as.table(data_cor))
    temp_degree_corr <- subset(temp_degree_corr, abs(Freq) > 0) 

  temp_degree_corr <- temp_degree_corr[order(-abs(temp_degree_corr$Freq)),] 
  
  
  #tried to find the the features in the most correlation
