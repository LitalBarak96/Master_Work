...
#moving avarge for multipul movies together for visual
...
#check if the component csv is exist,if not,creat it  
filepath_csv_component<-paste0(dir[num_of_movie], '/',"componentSize.csv")

#if the component not exist
...
print(num_of_movie)
 ...
 all_movie_change<-bind_cols(all_movie_change,all_changes_current_movie)
}
  
  #there is no colomof frame in no one

library(zoo)


#get the seceond colom and the next ones (just not the first which is the frames) and do roll mean on each colom
#removed the colom with the frame number
rollmeans_eachcolom<-NULL
for(coloms in 1:number_of_movies){
  means <- sapply(c("center"),
                function(x)zoo::rollmean(all_movie_change[,coloms],1000,align = x, na.pad = TRUE))
  
  rollmeans_eachcolom<-cbind(rollmeans_eachcolom,means)
}
...
library(tidyr)
rollmeans_eachcolom<-as.data.frame(rollmeans_eachcolom)
#remove na that was for padding
means_no_na<-rollmeans_eachcolom %>% drop_na()
#add the frames
...
melted_no_na<- melt(data = means_no_na, id.vars = "frame")


all_movie_change$frame<-1:nrow(all_movie_change)
colnames(all_movie_change)<-c(c(1:number_of_movies),"frame")
melted_original_comp<- melt(data = all_movie_change, id.vars = "frame")


movingavg_all_movie<-ggplot(melted_no_na, aes(frame,value,colour = variable)) + 
  geom_point() + 
  geom_line() +
  facet_wrap(~variable,nrow =number_of_movies)
