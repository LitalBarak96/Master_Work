...
group_name<-"Males_Singels"

my_path<-paste0("D:\\allGroups\\",group_name)
all.the.files <- dir(my_path, recursive=TRUE, pattern="^all_frames_degree_new.csv$" )

all.the.files<-paste0(my_path,"\\",all.the.files)
library(purrr)
...
colnames(wo_frame_and_num_group)<-1:length(all.the.files)
...
p<-ggplot()+geom_line(data = wo_frame, mapping = aes(x = X, y = Value,group=group,color=group)) +geom_point(data=mean_everyrow, aes(x=Frame, y=Value), color='red', size = 3,alpha = 0.15)
 ...
 p<-p+theme(text = element_text(size=10),
        axis.text.x = element_text(angle=90, hjust=1)) +ggtitle(paste0("every frame for all movies ",group_name)) +
