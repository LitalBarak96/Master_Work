...
debbug_path_color<-"F:/test/dir_and_color.xlsx"
...
for (j in 1:num_of_pop){
  colors_of_groups$X1[j]<-rgb_2_hex(allColorData[j,2:4])
}


for(n in 1:num_of_pop){
  current_dir<-dir[n,1]
  setwd(current_dir)
  
  dir<-list.dirs(recursive = F)
  print(dir)
  color<-colors_of_groups$X1[n]
  order_index<-1

  df <- (readMat(paste0(dir[order_index], '/','Allinteraction.mat'))) # read each MAT file
  mat_all_inter<-matrix(df[["new.interactionFrameMatrix"]],nrow = number_of_flys, ncol = number_of_flys)
  ...
  #informaction on each fly for 10 flys
