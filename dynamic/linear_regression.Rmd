...
summary(lm_fliesn)
      predi_n_close<-as.data.frame(predict(lm_fliesn))
        ...
        lm1<-lm(degree ~ .,data =data_non_scaled)
    summary(lm1)
...
all_other_features_test<-read.csv("D:/allGroups/Females_Mated/Assa_Females_MatedN_Unknown_RigA_20220208T124803/per_framefeatures_sum_allflies.csv")[1:26997,]

degree_csv_test<-read.csv("D:/allGroups/Females_Mated/Assa_Females_MatedN_Unknown_RigA_20220208T124803/all_frames_degree.csv")


  degree_csv_test<-degree_csv_test[1:26997,c(1,3)]
  colnames(degree_csv_test)<-c("Frame","degree")
  degree<-degree_csv_test$degree
scaled_all_other_features<-scale(all_other_features_test)
    #combined_degree_and_features<-cbind(scaled_all_other_features,degree)
#nonscaled
non_scaledcombined_degree_and_features<-cbind(all_other_features,degree)
    data_non_scaled_only_features_test<-all_other_features_test
  data_non_scaled_only_features_test$predicted.y <- predict.lm(lm1, newdata=data_non_scaled_only_features_test)
  ...
  library(tidyr)
  library(ggplot2)
a=data_non_scaled_only_features_test$predicted.y
b=(degree)
time=degree_csv$Frame
...
all.the.data_all_features <- 
    test_file_names %>%  map_dfr( ~ read.csv(.x) )
