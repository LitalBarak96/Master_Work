...
all_cluster_coff_local_zeros <- replace(all_cluster_coff_local, is.na(all_cluster_coff_local), 0)

all_cluster_coff_local_minus_1 <- replace(all_cluster_coff_local, is.na(all_cluster_coff_local), -1)


colnames(all_cluster_coff_local_minus_1)<-1:10
rownames(all_cluster_coff_local_minus_1)<-1:(length(numbers)-1)
...
temporary_for_trying_things<-all_cluster_coff_local_minus_1

all_cluster_coff_local_minus_1<-temporary_for_trying_things




all_cluster_coff_local_minus_1<-as.data.frame(all_cluster_coff_local_minus_1)
all_cluster_coff_local_minus_1$frame<-rownames(all_cluster_coff_local_minus_1)
all_cluster_coff_local_minus_1 <- melt(all_cluster_coff_local_minus_1 ,  id.vars = 'frame',variable.name = 'series')
...
cluster_coff_all_flys<-all_cluster_coff_local_minus_1 %>%
  ggplot( aes(x=frame, y=value, group=series, fill=series)) +
    geom_area() +
    theme(legend.position="none") +
    ggtitle("cluster_coff each fly over time") +
    theme(
