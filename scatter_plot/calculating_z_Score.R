...
for (i in 2:length(temp.df)){
  ...
  avg_all_pop.df<-data.frame(file=levels(temp.df[[i-1]]),mean_all_pop=((first.df$value[i]+second.df$value[i]+first.df$value[i])/3)) # create average per condition
...
for (i in 1:number_of_features){
  ...
  new_avg.df[i,]$mean_all_pop = (first.df$value[i]+second.df$value[i]+first.df$value[i])/3
