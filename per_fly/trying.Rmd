...
for runing this code we need to creat combin per dly which is in tsne file and calls main per fly
...
pathCsv<-"F:/hadar/combined_per_fly.csv"
...
plots_per_movie(Data6,3,dirname(pathCsv))
...
path_test<-"D:/hadar/20230305/all_grouped"


pathCsv<-paste(path_test,"/","averages per fly per movie.csv",sep = "")
per_fly_data<-read.csv(pathCsv)

sub_dir<-list.dirs(path_test,recursive = F)
 ...
 for(j in 1:length(sub_dir)){
  ...
  list_of_flys<-get_fly_from_user(sub_dir[j])
