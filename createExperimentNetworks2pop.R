...
calculateGroupParams <- function(fileNames, maxNumberOfInteration) {
  ...
  for (i in 1:length(fileNames)) {
    ...
    if (maxNumberOfInteration > 0) {
      #normalization to the number of the max interaction
  ...
  #it need to return tuple of this values with lenght and number,maybe not beacuse we run this twice but each param have many values as the 
  return(list(density, modularity, sdStrength, strength, betweenness))
}
