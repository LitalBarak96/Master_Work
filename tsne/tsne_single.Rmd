...
paths_of_groups <- as.data.frame(read.xlsx(debbug_path_color))

library(stringr)
library(dplyr)

dirs<-paths_of_groups %>%
  filter(str_detect(groupNameDir, "Single"))
```

## R Markdown
#tsne for female single and male single

```{r all sub groups}
#all groupes
  library(stringr)
  library(openxlsx)
 # dirs <- as.data.frame(read.xlsx("F:/allGroups/single_color.xlsx"))
...
#ggsave(file="male_vs_female_single.pdf",device="pdf",path="F:/allGroups",units="cm",width=20,height=20,dpi="retina")
